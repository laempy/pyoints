...
# We recieve a LasRecords epresenting the point cloud. We can treat it like a numpy record array with additional features. So we inspect its properties first.
...
# The LAS file provides some altitude values. But we are interrested in the height above ground instead. So, we select points representing the ground first, to fit a digital elevation model (DEM). Using the DEM we calculate the height above ground for each point.
...
# We have recieved a list of point indices which is used to select the desired representative ground points. So, we update the classification field for these points and plotthem.
...
# Finally we calculate the height above ground. And add a corresponding height attribute to the point cloud.
...
# Now we try tofilter the points subsequently until only points associated with stems remain.
...
# Due to the unnecessary high point density we filter the point cloud using a duplicate point filter. Only a subset of points with a point distance of at least 0.1 m is kept.
...
# Let's take a look at the remaining point cloud. To recieve a nice plot, we define the axis limits first.
...
# We can see four trees with linear stems. But there are a lot of points associated with branches or leaves. We assign class 23 to the selected points for later visualisation.
...
# That's mutch better, but we are interrested in the linear shape of the stems only. So, we filter with a radius of 1 m to remove similar points. This results in a point cloud with point distances of at least 1 m.
...
# After applying the filter the stems are clearly visible. Thus, we can proceed with the actual detection of the stems.

# ## Identification of stem clusters
# We can detect the stems easily by clustering the previously filtered points. For this purpose we apply use the DBSCAN algorithm.
...
# In the next step, we remove small clusters and unassigned points. In addition, we add an additional field, providing the tree ID. to the point cloud 
...
# Finally, we can fit a vector to each stem.
...
# Finally, we determinate the tree root coordinates by calculating the intersection points with the surface.
