...
class PostgisHandler(psycopg2._psycopg.connection):
    ...
    def __init__(self,connectionString):
        ...
        self.set_isolation_level(psycopg2.extensions.ISOLATION_LEVEL_AUTOCOMMIT)
    ...
    @staticmethod
    def sFLOAT(value):
        return PostgisHandler._NULL if value is None or np.isnan(value) else PostgisHandler._sFLOAT%value
    
    @staticmethod
    def sARRAY(arr):
        values=','.join(np.array(arr).astype(str))
        return PostgisHandler._NULL if len(arr)==0 or np.isnan(arr).sum()==len(arr) else PostgisHandler._sARRAY%values
    
    @staticmethod
    def sSTRINGARRAY(arr):
        values="'%s'"%"','".join(np.array(arr).astype(str))
        return PostgisHandler._NULL if len(arr)==0 else PostgisHandler._sARRAY%values
    
    
    @staticmethod
    def sINTEGER(value):
        return PostgisHandler._NULL if value is None or np.isnan(value) else PostgisHandler._sINTEGER%value
    
    @staticmethod
    def sBOOLEAN(value):
        return PostgisHandler._NULL if value is None or np.isnan(value) else PostgisHandler._sSTRING%value
    ...
    def sMULTIPOINT(coords,srid):
        values='('+'),('.join([' '.join(coord) for coord in coords.astype(str)])+')'
        return PostgisHandler._sMULTIPOINT%(srid,values)
    
    @staticmethod
    def sLINESTRING(coords,srid):
        values=','.join([' '.join(coord) for coord in np.array(coords).astype(str)])
        return PostgisHandler._sLINESTRING%(srid,values)
    
    @staticmethod
    def sCONVEXHULL(coords,srid):
        if coords is not None and len(coords.shape)==2 and coords.shape[0]>2 and coords.shape[1]>1:
            ext=Extent(coords)
    ...
    def colNames(self,query):
        columnQuery='SELECT * FROM (%s) AS "count_foo" LIMIT 1;'%(query.rstrip(';'))
    ...
    def insertStream(self,table,stream,colNames=None,bulk=5000,schema='public'):
    ...
    def updateStream(self,table,stream,colNames,idField,bulk=5000,schema='public'):
        # Stream of precompiled attributes
    ...
    def upsertStream(self,table,stream,colNames,idField,bulk=5000,schema='public'):
        fields=('","'.join(colNames))       
        
        upsertQuery='INSERT INTO "%s"."%s" ("%s","%s") '%(schema,table,idField,fields)
    ...
    def __call__(self,sqlStatement,where=None,bulk=None,schema=None):
        if where is not None:
            ...
            sqlStatement='SELECT * FROM (%s) AS "tempQuery" WHERE %s;'%(sqlStatement,where)
        
        if bulk is not None:
            stream=iter(sqlStatement.split(';\n'))
            converter=lambda query: '%s;'%query
    ...
    def load(self,query,npDtypes=[],converters={},where=None,bulk=None):
        ...
        if bulk is None:
            return self.recArrayFromCursor(cursor,npDtypes=npDtypes,converters=converters)
        else:
            def gen():
                while True:
                    recArray=PostgisHandler.recArrayFromCursor(cursor,npDtypes=npDtypes,converters=converters,bulk=bulk)     
                    if recArray is None:
                        break
                    yield recArray
            return gen()
    ...
    def recArrayFromCursor(cursor,npDtypes=[],converters={},bulk=None):
        assert isinstance(converters, dict), 'Converters have to be a dictionary'
    ...
    def importShape(self,fileName,epsg,schema='public',bulk=500,outTable=None):
        ...
        command='shp2pgsql -k -I -s %i %s "%s"."%s"  '%(epsg,fileName,schema,outTable)
        print command
        query=shell(command)
        print 'insert DB'
